...
function renderNotifications() { const list = document.querySelector('#notificationList'); const notices = tasks.filter(task => task.progress < 100 && (task.days <= 1 || task.category === 'critical')).sort((a, b) => b.score - a.score).slice(0, 4); list.replaceChildren(...(notices.length ? notices.map(task => { const item = make('button', 'notification-item'); item.addEventListener('click', () => { setView('overview'); document.querySelector('#notificationPopover').hidden = true; document.querySelector('#notificationsButton').setAttribute('aria-expanded', 'false'); }); item.append(make('span', 'notification-item-dot'), make('span', '', `${task.name} · ${formatDeadline(task.deadline)}`)); return item; }) : [make('p', 'notification-empty', 'You’re all caught up for now.') ])); }
function renderAll() { enrichTasks(); renderTasks(); renderStats(); renderNextAction(); renderUpcoming(); renderNotifications(); }
...
document.querySelector('#headerAvatar').addEventListener('click', () => setSettings(true));
...
document.querySelector('#notificationsButton').addEventListener('click', () => { const popover = document.querySelector('#notificationPopover'); popover.hidden = !popover.hidden; document.querySelector('#notificationsButton').setAttribute('aria-expanded', String(!popover.hidden)); });
document.querySelector('#closeNotifications').addEventListener('click', () => { document.querySelector('#notificationPopover').hidden = true; document.querySelector('#notificationsButton').setAttribute('aria-expanded', 'false'); });
